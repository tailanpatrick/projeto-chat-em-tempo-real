...
connectDB()
    ...
    wss.on('connection', (ws) => {
        ...
        ws.on('message', async (data) => {
            ...
            try {
                ...
                console.log('Mensagem salva e retransmitida:', savedMessage);
                ...
                console.error('Erro ao processar a mensagem do WebSocket:', err.message);
            }
        });
        ws.on('close', () => {
    ...
    app.listen(expressAppPort, () => {
        console.log(`Servidor Express (API REST) rodando na porta ${expressAppPort}`);
    });
})
    .catch((error) => {
    console.error('ERRO CRÍTICO: Falha ao iniciar a aplicação devido a erro de conexão com o banco de dados.', error.message);
});
